...
func (h *hnsw) Add(id uint64, vector []byte) error {
	if len(vector) == 0 {
...
func (h *hnsw) insertInitialElement(node *vertex, nodeVec []byte) error {
	h.Lock()
	defer h.Unlock()
...
func (h *hnsw) insert(node *vertex, nodeVec []byte) error {
	wasFirst := false
